...
void SettingsEditorController::loadSettings()
{
    ...
    //Flight Presets
    setLowSpeed(settings.value(key_lowSpeed, 13).toDouble());
    setMediumSpeed(settings.value(key_mediumSpeed, 14).toDouble());
    setHighSpeed(settings.value(key_highSpeed, 15).toDouble());
    setLowAltitude(settings.value(key_lowAltitude, 30).toDouble());
    setMediumAltitude(settings.value(key_mediumAltitude, 40).toDouble());
    setHighAltitude(settings.value(key_highAltitude, 50).toDouble());

    //Flight Settings
    setTurnaroundDistance(settings.value(key_turnaroundDistance, 50).toDouble());
    setLoiterRadius(settings.value(key_loiterRadius, 55).toDouble());
